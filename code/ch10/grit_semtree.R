...
grit.kev <- read.csv("code/ch10/data_kev.csv", header = T, sep = ",")
# set of models
...
cfa2 <- mxModel("Model",
  ...
  latentVars = c("f1", "f2"),
  mxPath(
    from = "f1", to = c("GS2", "GS3", "GS5", "GS7", "GS8", "GS9", "GS11"),
    free = c(F, T, T, T, T, T, T), values = c(1, 1, 1, 1, 1, 1, 1), labels = c("l1", "l2", "l3", "l4", "l5", "l6", "l7")
  ),
  mxPath(
    from = "f2", to = c("GS1", "GS4", "GS6", "GS8", "GS9", "GS10", "GS11", "GS12"),
...
sapply(semtree:::getNodeList(tree.out), function(x) {
  c(x$node_id, nrow(x$model$data$observed))
})
...
tree.out.focus <- semtree(out.cfa2, data.frame(grit.train),
  control = my.control3,
...
tree.out.inv <- semtree(out.cfa2, data.frame(grit.train),
  control = my.control3,
  constraints = semtree.constraints(global.invariance = c(
...
tree.out.focus.means <- semtree(out.cfa3, data.frame(grit.train.scale),
  control = my.control3,
...
tree.out.inv.means <- semtree(out.cfa3, data.frame(grit.train.scale),
  control = my.control3,
  constraints = semtree.constraints(global.invariance = c(
    "l1", "l2", "l3", "l4", "l5", "l6", "l7",
    "l11", "l22", "l33", "l44", "l55", "l66", "l77", "l88",
